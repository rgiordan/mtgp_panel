...
FitIfNotSaved <- function(fit_fun, filename, force=FALSE) {
...
# Test
#foo <- FitIfNotSaved(function() { return(1) }, "/tmp/test.RDS", force=FALSE)
...
FitFun <- function() {
  ...
  return(fit_ranks_pois)  
}
fit_ranks_pois <- FitIfNotSaved(FitFun, "mtgp_fits_poisson.RDS", force=force_refit)
...
post_pred_ranks_pois <- lapply(posts_pois, get_post_pred_samples, unit=State, 
                               time=year,
                               data = crimes %>% filter(year >= start_year))
pred_time <- Sys.time() - tic
print(pred_time) # Not too long compared with MCMC
...
fit_ranks_pois <- FitIfNotSaved(FitFun, "mtgp_fits_poisson_adjusted.RDS", force=force_refit)
...
FitFun <- function() {
  ...
  fit_ranks_gauss <- lapply(ranks, fit_mtgp_normal, outcome = homicide,
                      ...
                      unit = State, time = year, t_int = 2007,
                      data = crimes %>% filter(year >= start_year))
  return(fit_ranks_gauss)  
}
fit_ranks_gauss <- FitIfNotSaved(FitFun, "mtgp_fits_homoskedastic.RDS", force=force_refit)
...
fit_ranks_gauss <- FitIfNotSaved(FitFun, "mtgp_fits_homoskedastic_adjusted.RDS", force=force_refit)
#fit_ranks_gauss[[1]]$output(1)
...
pivot_names <- names(df)[4:ncol(df)]
df %>%
    filter(year >= 1996) %>%
    filter(state_abbr != 'DC') %>%
    pivot_longer(pivot_names, names_to='crimetype', values_to='count') %>%
...
model_gpbffg <- cmdstan_model(stan_file = filebf0, include_paths = ".", quiet=FALSE)
...
FitFun <- function() {
  fit_gpcovfg <- model_gpbffg$sample(
    data=standata_gpbffg,
    iter_warmup=500, iter_sampling=500,
    chains=4, parallel_chains=4, adapt_delta=0.9, max_treedepth = 13)
  return(fit_gpcovfg)
}
fit_gpcovfg <- FitIfNotSaved(FitFun, "mtgp_fit_multi.RDS", force=force_refit)
